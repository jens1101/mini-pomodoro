...
export class CountdownTimerElement extends HTMLElement {
  /**
   * The element in which the current countdown time is being displayed.
   * @type {HTMLElement}
   */
  #timeDisplayElement

  /**
   * The button element that will start the countdown.
   * @type {HTMLElement}
   */
  #startCountdownElement

  /**
   * The button element that will stop the countdown.
   * @type {HTMLElement}
   */
  ...
  /**
   * A promise that will only resolve once this component has finished
   * initialising.
   * @type {Promise}
   */
  #setupPromise
  ...
  constructor () {
    ...
    this.#setupPromise = ComponentHelper.initWithUrl(this, templateUrl)
      .then(template => {
        this.shadowRoot.appendChild(template.stylesheets.get(1))
  ...
  startCountdown () {
    ...
    this.#countdownTimer.startCountdown(this.#dispatchStartEvent,
      this.#countdownComplete, this.#displayTime)
  }
