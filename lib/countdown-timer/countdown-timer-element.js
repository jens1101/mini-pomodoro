...
export class CountdownTimerElement extends HTMLElement {
  ...
  constructor () {
    ...
    this._durationMs = CountdownTimerElement.DEFAULT_DURATION_MS

    const templateUrl = new URL('./index.html', import.meta.url)
    this._setupPromise = ComponentHelper.initWithUrl(this, templateUrl)
      .then(() => {
        this._timeDisplayElement = this.shadowRoot.getElementById('time-display')

        this._startCountdownElement = this.shadowRoot.getElementById('start-countdown')
        this._startCountdownElement.addEventListener('click', () => this.startCountdown())

        this._stopCountdownElement = this.shadowRoot.getElementById('stop-countdown')
  ...
  resumeCountdown (startTimestamp) {
    this._tick(startTimestamp, Date.now() - startTimestamp)
  }
  ...
  _tick (startTimestamp, elapsedMs) {
    ...
    if (inaccuracy > CountdownTimerElement.TICK_SIZE_MS) {
      // This is the remainder of millisecons after the timer would be adjusted.
      // This is used so that the timer still self-adjusts.
      const mod = inaccuracy % CountdownTimerElement.TICK_SIZE_MS

      // This updates the elapsed millisecons to jump to the most recent tick
    ...
    // was an inaccuracy.
    nextTickSize = CountdownTimerElement.TICK_SIZE_MS - inaccuracy
    ...
    this._currentTimeoutReference = setTimeout(this._tick.bind(this),
      nextTickSize,
      startTimestamp,
      elapsedMs + CountdownTimerElement.TICK_SIZE_MS)
  }
