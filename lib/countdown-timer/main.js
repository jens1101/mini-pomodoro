/* globals HTMLElement, customElements, CustomEvent */

class CountdownTimer extends HTMLElement {
  constructor () {
    super()
    this._intervalReference = null
  }

  // Specify observed attributes so that `attributeChangedCallback` will work
  static get observedAttributes () { return ['duration', 'start-text', 'stop-text'] }

  connectedCallback () {
    // Note: `document.currentScript.ownerDocument` doesn't work when this is a module. This is
    // because the variable gets garbage collected before the script runs. It would be best if we
    // find an alternative.
    const templateDocument = document.currentScript.ownerDocument

    /*
    I need the current document to get the template and the base URI
    Alternatives:
    - Use AJAX to get the template and the style sheet and inject them. This isn't really vanilla,
      it kinda defeats the purpose :/
    - I canuse a style tag, but that isn't a nice separation of concerns. It also still makes us
      dependant on currentScript :/
    - I can declare everything using pure JS. This means there is no mixing of languages, a single
      point of entry, we can use JS modules, and I don't even need to use HTML imports. However,
      this isn't excatly good separation of concerns, would also be unfeasable for larger DOM
      strucutres, and kinds defeats the purpose of web components :/
    - I can use Polymer. It isn't vanilla, but I'd rather use it if I can't use vanilla :/
    - It seems to me the best practise at the moment is to use the JS module system to import/export
      components. I seriously think Polymer is the way to go, especially now that v3 is released.
    */

    const templateElement = templateDocument.querySelector('template')
    const template = templateElement.content.cloneNode(true)

    this._timeDisplayElement = template.getElementById('time-display')

    const startCountdownButton = template.getElementById('start-countdown')
    startCountdownButton.addEventListener('click', () => this.startCountdown())

    const stopCountdownButton = template.getElementById('stop-countdown')
    stopCountdownButton.addEventListener('click', () => this.stopCountdown())

    const stylesheetLink = document.createElement('link')
    stylesheetLink.setAttribute('rel', 'stylesheet')
    stylesheetLink.setAttribute('href', `${templateDocument.baseURI}test.css`)
    stylesheetLink.setAttribute('type', 'text/css')

    const shadowRoot = this.attachShadow({mode: 'open'})
    shadowRoot.appendChild(stylesheetLink)
    shadowRoot.appendChild(template)
  }

  attributeChangedCallback (name, oldValue, newValue) {
    // TODO: add cases for the start and stop button text
    switch (name) {
      case 'duration':
        // TODO: possibly add support for duration strings via momentjs
        if (isNaN(newValue)) {
          this._duration = 25 * 60 // Default to 25 minutes
        } else {
          this._duration = parseInt(newValue)
        }
    }
  }

  startCountdown () {
    if (this._intervalReference) {
      this.stopCountdown()
    }

    let seconds = this._duration
    this._displayTime(seconds)

    // TODO: This implementation is prone to continuation errors. Rather use a `startTime` timestamp.
    // TODO: Handle duration changes via the attribute
    this._intervalReference = setInterval(() => {
      --seconds
      this._displayTime(seconds)

      if (seconds <= 0) {
        this.stopCountdown()

        // TODO: add timestamp to this event
        const event = new CustomEvent('countdowncomplete')
        this.dispatchEvent(event)
      }
    }, 1000)

    // TODO: add timestamp to this event
    const event = new CustomEvent('countdownstart')
    this.dispatchEvent(event)
  }

  stopCountdown () {
    clearInterval(this._intervalReference)
    this._intervalReference = null
    this._timeDisplayElement.textContent = '00:00'

    // TODO: add timestamp to this event
    const event = new CustomEvent('countdownstop')
    this.dispatchEvent(event)
  }

  _displayTime (duration) {
    // TODO: use moment.js to format this properly

    let minutes = Math.floor(duration / 60)
    let seconds = Math.floor(duration % 60)

    if (minutes < 10) { minutes = `0${minutes}` }
    if (seconds < 10) { seconds = `0${seconds}` }

    this._timeDisplayElement.textContent = `${minutes}:${seconds}`
  }
}

customElements.define('countdown-timer', CountdownTimer)
