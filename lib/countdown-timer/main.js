...
export class CountdownTimer extends HTMLElement {
  ...
  async connectedCallback () {
    ...
    template.element.getElementById('start-countdown')
      .addEventListener('click', () => this.startCountdown())

    template.element.getElementById('stop-countdown')
  ...
  /**
   * Lifecycle callback. Gets called whenever the component's HTML attributes are changed.
   */
  ...
  /**
   * Starts the countdown. This first stops any existing countdown and then stats it anew. This
   * dispatches a custom event on this element called "countdownstart".
   */
  startCountdown () {
  ...
  /**
   * Stops the current countdown. This sets the timer back to zero and cancels any existing timeout.
   * This dispatches a custom event on this element called "countdownstop".
   */
  stopCountdown () {
  ...
  /**
   * Private function which is actually responsible for the countdown. It displays the time in the
   * view and stops the countdown once it's done. This also dispatches a "countdowncomplete" event
   * on this element once it's done.
   * This function gets called to start the countdown and will recursively call itself until the
   * contdown is complete.
   * @private
   * @param {number} startTimestamp A Unix timestamp in millisecons when this timer was started.
   * @param {number} elapsedMs The number of millisecons that have *supposed to be* passed since
   * the starting timestamp. In reality more time may have passed due to the asynchronous nature of
   * `setTimeout`, but we use this parameters to correct such deviation.
   */
  _tick (startTimestamp, elapsedMs) {
    this._displayTime(this._durationMs - elapsedMs)

    if (elapsedMs >= this._durationMs) {
      // If more time has elapsed than the duration of the countdown timer then dispatch the
      // "countdowncomplete" event and stop the timer.
    ...
    // This code below accounts for any inaccuracies that may have accumulated and corrects them.

    // This gets the difference between how may milliseconds are *supposed to* have ellapsed and how
    // many have *actually* ellapsed. This is how much inaccuracy has accumulated so far.
    const inaccuracy = (Date.now() - startTimestamp) - elapsedMs
    // This gets the next tick size by subtracting the inaccuracy from the default tick size. The
    // next tick will therefore complete quicker.
    const nextTickSize = CountdownTimer.TICK_SIZE_MS - inaccuracy
    // This recursively calls the next `_tick` using a `setTimeout`. We use recursive `setTimeout`
    // calls instead of `setInterval`, because `setInterval` tends to be suspended by the browser
    // once the web page goes into the background. `setInterval` also tends to be more inaccurate,
  ...
  /**
   * Displays the current countdown time in the element in the format: "hh:mm:ss".
   * @private
   * @param durationMs The number of millisecons that are still left for this countdown timer.
   */
